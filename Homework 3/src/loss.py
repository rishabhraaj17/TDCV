...
class TripletAndPairLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        positive_distance = torch.sum((anchor - positive).pow(2), dim=1)
        negative_distance = torch.sum((anchor - negative).pow(2), dim=1)
        triplet_loss = torch.max(0.0, (1 - (negative_distance) / (positive_distance + self.margin)))
