...
% Arguments: 
% path - the path to a directory with images or path to a single image
...
I = NaN(max_points, 2, num_files);

for i=1:num_files
    
    fprintf('Image: %d \n', i);
    img_data = imread(fullfile(img_files(i).folder, img_files(i).name));
    fig = figure;
    image(img_data);
    hold on;
